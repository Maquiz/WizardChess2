...
public class GameMaster : MonoBehaviour {
			...
			if (Physics.Raycast (ray, out hit)) {
				...
				//if it is 
					...
					if (selectedPiece != null) {
						if (selectedPiece.checkPath (s.x, s.y)) {
	...
	//Game Creation
	private void createPiece (int piece, int color) { //1 pawn, 2 rook, 3 knight, 4 bishop, 5 queen, 6 king, 
			...
			for (int j = 0; j < size; j++) {
				if (i % 2 == 0) {
					if (j % 2 == 0) {
						...
						boardPos [i, j].GetComponent<Square> ().x = i;
						boardPos [i, j].GetComponent<Square> ().y = j;
